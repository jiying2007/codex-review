...
Codex Review Safe uses Codex Safe Core v4.3 efficiency primitives. The staged review now applies one total evidence budget across all chunks instead of allowing every chunk to consume the full cap independently. Higher-risk evidence is selected first when the total byte budget cannot cover every chunk, and any omission remains an explicit coverage gap so the verdict fails closed.
