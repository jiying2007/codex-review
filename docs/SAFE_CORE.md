...
Codex Review Safe vendors **Safe Core v1** from the canonical product-family source `jiying2007/codex-commit:safe-core-v1`.
...
- `upstream` checks that the locked manifest still matches the canonical source; CI runs this fail-closed drift gate.
- `sync` deliberately downloads the canonical manifest/files, verifies their hashes, updates the vendored copy and lock, and leaves the resulting diff for human review.

There is no Git submodule and no runtime network dependency on the canonical repository.
