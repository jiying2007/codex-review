...
Codex Review Safe 从产品族唯一规范来源 `jiying2007/codex-commit:main/src/codex-safe-core` vendor **Safe Core v1**。
...
- `upstream` 校验锁定版本是否仍与规范来源 `codex-commit/main` 一致；CI 使用这个 fail-closed 漂移门禁。
- `sync` 显式从 `codex-commit/main` 下载规范来源的 manifest/文件，先校验上游 hash，再更新 vendored 副本和 lock，最终差异仍需人工 Review 后提交。

不使用 Git submodule、不再维护专用 Safe Core 分支，插件运行时也不依赖规范仓库网络连接。
