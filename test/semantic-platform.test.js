...
assert.match(extensionSource,/REVIEW_EVIDENCE_PROTOCOL_VERSION = 2/);
...
assert.match(extensionSource,/result replay: validated judgment replayed for the current ReviewSubject/);
...
assert.doesNotMatch(extensionSource,/getReplayByEvidenceKey/,'judgment replay must be resolved only after the current ReviewSubjectKey is recomputed');
...
const putEvidenceBody=cacheSource.match(/async function putEvidence[\s\S]*?\n  }\n\n  function getReplay/)?.[0]||'';
assert.doesNotMatch(putEvidenceBody,/reviewSubjectKey/,'deterministic Evidence Cache must not persist judgment-domain ReviewSubjectKey');
assert.match(extensionSource,/reviewSubjectKey = computeReviewSubjectKey[\s\S]*?reviewCache\.getReplay\(repoRoot, reviewSubjectKey\)/,'replay lookup must use the freshly resolved current ReviewSubjectKey');
