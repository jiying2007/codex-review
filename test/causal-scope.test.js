...
assert.throws(() => validateCausalAnchor('pt_common.c', 800, staged, changed), /causal anchor span is not entirely on exact changed lines/);
assert.throws(() => validateCausalAnchor('pt_common.c', 101, staged, changed, 102), /causal anchor span is not entirely on exact changed lines/);
...
assert.match(semanticSource,/validateCausalAnchor\(normalized\.file,normalized\.line,stagedPathSet,changedLineRanges,normalized\.endLine\)/,'semantic review must validate the full causal anchor span');
