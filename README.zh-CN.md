...
Codex Review Safe 3.0 固定使用 **Codex Safe Core 3.0.1**，pin 到 `e6e25b502aa35a079f660346785cf283fe293b6d`。仓库策略统一使用 HEAD 中已提交的 `.codex-safe.json`（`schemaVersion: 3`）；Policy v2 明确拒绝，不保留兼容 parser。
...
- Review Receipt v3 使用 `git-index` subject，绑定 HEAD/index/diff/policy 指纹以及 quality/readiness/mechanical/coverage verdict；
