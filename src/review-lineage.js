...
const { normalizeFsPath, normalizeGitPathForComparison } = require('./review-support');
...
function snapshotFinding(finding) {
  return {
    ...
    file: normalizeGitPathForComparison(finding.file || ''),
...
function transitionBetween(previous, current, seenIds = new Set()) {
  if (!previous) {
    return {
      previousReviewKey: '',
      newIds: current.map(item => item.stableFindingId).filter(Boolean),
      unchangedIds: [], changedIds: [], fixedIds: [], reintroducedIds: [], likelyFixInducedIds: [],
      previousCount: 0, currentCount: current.length
    };
  }
  ...
  for (const [id, before] of prev) {
    ...
    if (before.severity === after.severity && before.evidenceDigest === after.evidenceDigest) unchangedIds.push(id);
  ...
  return {
    previousReviewKey: '', newIds, unchangedIds, changedIds, fixedIds, reintroducedIds, likelyFixInducedIds,
...
function createReviewLineageStore(globalState) {
  ...
  function restore() {
    ...
    for (const [repo, sessions] of Object.entries(stored)) {
      ...
      const valid = sessions.filter(session => session && typeof session.sessionKey === 'string' && Array.isArray(session.runs))
        .map(session => ({ ...session, runs: session.runs.slice(-MAX_RUNS_PER_SESSION) }))
        .slice(0, MAX_SESSIONS_PER_REPO);
  ...
  function latest(repoRoot, sessionKey) {
    const session = getSession(repoRoot, sessionKey);
    return session?.runs?.[session.runs.length - 1] ? clone(session.runs[session.runs.length - 1]) : null;
  }
  async function record(repoRoot, input) {
    const repoKey = normalizeFsPath(repoRoot);
    const values = sessions(repoRoot).map(clone);
    let session = values.find(item => item.sessionKey === input.sessionKey);
    if (!session) {
      session = { sessionKey: input.sessionKey, createdAt: new Date().toISOString(), phase: String(input.phase || 'unspecified'), runs: [] };
      values.unshift(session);
    }
    ...
    const lineage = {
      ...
      runNumber,
      previousReviewKey: transition.previousReviewKey,
      transition,
    ...
    byRepo.set(repoKey, values.slice(0, MAX_SESSIONS_PER_REPO));
    await flush();
