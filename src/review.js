...
function parseCodexJsonl(stdout) {
  ...
  for (const line of String(stdout || '').split(/\r?\n/).filter(Boolean)) {
    let event; try { event = JSON.parse(line); } catch { throw new Error(t('Codex --json returned invalid JSONL.')); }
...
function normalizeFinding(finding, stagedPathSet, changedLineRanges) {
  ...
  const severity = String(finding.severity || ''); if (!(severity in SEVERITY_ORDER)) throw new Error(t('Invalid severity: {0}', severity));
...
function validateReviewResult(value, options, stagedPaths, changedLineRanges) {
  ...
  const summary = String(value.summary || '').trim(); if (summary.length > 1200) throw new Error(t('Summary is too long.'));
  if (!Array.isArray(value.findings)) throw new Error(t('Findings must be an array.')); if (value.findings.length > options.maxFindings) throw new Error(t('The number of findings exceeds the configured limit.'));
  const stagedPathSet = new Set(stagedPaths.map(normalizeGitPathForComparison)); const findings = []; const suppressedFindings = []; const rejectedFindings = [];
  value.findings.forEach((rawFinding, index) => { try { const finding = normalizeFinding(rawFinding, stagedPathSet, changedLineRanges); if (finding.confidence < options.confidenceThreshold) suppressedFindings.push(finding); else findings.push(finding); } catch (error) { rejectedFindings.push({ index, reason: String(error?.message || error).slice(0, 300) }); } });
...
function firstChangedLine(changedLineRanges, path) { const ranges = changedLineRanges.get(normalizeGitPathForComparison(path)); return ranges?.[0]?.start || null; }
...
function consolidateReviewResults(results, options, stagedPaths, changedLineRanges, evidence) {
  ...
  return { summary: summaries.join('\n\n').slice(0, 1200), verdict: computeVerdict(uncapped, coverageComplete), qualityVerdict, readinessVerdict, mechanicalGate: mechanical.violations.length ? 'fail' : 'pass', coverageVerdict: coverageComplete ? 'complete' : 'incomplete', coverageGaps: [...new Set(coverageGaps)], cannotVerify: ['requirements', 'tests'], findings, suppressedFindings, rejectedFindings, mechanicalViolations: mechanical.violations, modelFindingCount, truncatedFindingCount: Math.max(0, uncapped.length - findings.length) };
}
function buildReviewInputMeta(snapshot, diffFingerprint, diffBytes, stagedPaths, unstagedPathSet, executionMeta = {}) {
  ...
  return { headOid: snapshot?.headOid || '<unknown>', indexFingerprint: snapshot?.indexFingerprint || '<unknown>', diffFingerprint: diffFingerprint || '<unknown>', diffBytes: Number(diffBytes) || 0, stagedFileCount: stagedPaths.length, unstagedOverlayPaths: overlays, codexVersion: executionMeta.codexVersion || 'unknown', model: executionMeta.model || 'cli-default', policySource: executionMeta.policySource || 'head-default', policyFingerprint: executionMeta.policyFingerprint || '<none>', coverageVerdict: executionMeta.coverageVerdict || 'incomplete' };
}
function createReviewReceipt(review, reviewInputMeta, now = new Date()) {
  return validateReviewReceipt({
    schemaVersion: REVIEW_RECEIPT_SCHEMA_VERSION, kind: 'codex-review',
    subject: { type: 'git-index', headOid: reviewInputMeta.headOid, indexFingerprint: reviewInputMeta.indexFingerprint, stagedFileCount: reviewInputMeta.stagedFileCount },
    diffFingerprint: reviewInputMeta.diffFingerprint, policyFingerprint: reviewInputMeta.policyFingerprint,
    reviewSubjectFingerprint: reviewInputMeta.reviewSubjectKey, evidenceManifestDigest: reviewInputMeta.evidenceManifestDigest,
    qualityVerdict: review.qualityVerdict, readinessVerdict: review.readinessVerdict, mechanicalGate: review.mechanicalGate, coverageVerdict: review.coverageVerdict,
    model: reviewInputMeta.model || 'cli-default', codexVersion: reviewInputMeta.codexVersion || 'unknown', createdAt: now.toISOString()
  });
}
...
module.exports = { computeVerdict, parseChangedLineRanges, lineInChangedRanges, outputSchema, buildPrompt, parseCodexJsonl, normalizeFinding, validateReviewResult, deterministicReview, consolidateReviewResults, buildReviewInputMeta, createReviewReceipt, shortFingerprint, severityPasses };
