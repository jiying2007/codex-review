...
function formatReviewTime(value, language = 'en') {
  ...
  let displayTime;
  try { displayTime = formatDisplayTime(value, { includeUtc: true }); } catch { return ''; }
...
function buildReviewReport(review, options, publishMeta, reviewInputMeta = {}) {
  ...
  if (review.suppressedFindings?.length) {
    ...
    for (const finding of review.suppressedFindings.slice(0, 20)) lines.push(`- ${finding.file || '?'}:${finding.line || '?'} [${finding.side || 'new'}] [${finding.category || 'other'}] ${finding.suppressionReason || finding.verificationStatus || 'suppressed'} — ${finding.title || finding.description || ''}`);
    lines.push('');
  }

  visibleFindings.forEach((finding, index) => {
    lines.push(`${index + 1}. [${finding.severity.toUpperCase()}] [${finding.category}] ${finding.file}:${finding.line} [${finding.side || 'new'}]`);
